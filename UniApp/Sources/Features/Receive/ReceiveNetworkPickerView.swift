...
/// Step 2 of the Receive sheet — the per-token network picker. Each
/// network row shows the wallet's real balance of this token ON that
/// network; rows are sorted balance high→low then tx-count high→low. A
/// native search bar filters by network name. Logos go through the cached
/// `CoinMark`. Shares its row + sort with the Send network picker.
