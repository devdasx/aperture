...
/// **zkSync Era connector — a fully independent EVM module.**
///
/// A FULLY INDEPENDENT module for `.zkSync` (chainId `0x144` / 324):
/// its own `eth_getBalance`, its own Multicall3 `balanceOf` token
/// batching, its own `eth_getLogs` token-transfer history + Blockscout
/// `txlist` native history. It owns its request shapes + parsing
...
/// never a raw `URLSession`. Endpoints come from
/// `RPCRegistry.endpoints(for: .zkSync)` (matter-labs
/// `mainnet.era.zksync.io` primary, `zksync.drpc.org`, keyed 1rpc
/// `zksync2-era`).
///
/// **Copied from `EthereumConnector` (the EVM template), per user
/// direction.** The per-chain duplication is intentional: zkSync's
/// endpoint quirks, decimals fixes, or parsing tweaks never leak into a
/// sibling EVM connector and vice-versa. The only swaps from the
/// Ethereum reference are `chain = .zkSync` and `blockscoutHost =
...
///
/// **Live-verified (2026-06-16, `mainnet.era.zksync.io`).** `eth_chainId`
/// → `0x144`. Multicall3 IS deployed at the canonical
/// `0xcA11bde05977b3631167028862bE2a173976CA11` on zkSync Era (standard
/// Multicall3 bytecode; `aggregate3` decodes cleanly), so the batched
/// `eth_call` path is the primary; the per-token sequential fallback is
/// dead weight here but retained for parity with the template.
/// `eth_getBalance`/`eth_getTransactionCount`/`eth_blockNumber`/`eth_call
/// balanceOf`/`eth_getLogs`(Transfer) and the `zksync.blockscout.com`
/// `txlist` all returned valid responses against a real funded holder.
struct ZkSyncConnector: ChainConnector {
    let chain: SupportedChain = .zkSync
    let client: RPCClient

    private static let log = Logger(subsystem: "com.thuglife.aperture", category: "zksync-connector")
    ...
    ///
    /// zkSync Era's native gas token is ETH (18 decimals).
    ...
    ///
    /// zkSync's curated registry is USDC (`0x1d17…538D4`, 6dp) + USDT
    /// (`0x4932…2bA4C`, 6dp) — see `EVMTokenRegistry.tokens(for:
    /// .zkSync)`. Multicall3 is deployed at the canonical address on
    /// zkSync Era (live-verified), so the batched path is the primary.
    ///
    ...
    /// zkSync Era — live-verified), same per-token sequential fallback
    /// when Multicall3 isn't deployed (empty bytes / deterministic
    ...
    /// by `maxLogRange` chunking below. ~50k blocks lookback window —
    /// same window the EVM template uses across the fleet.
    private static let scanBlockRange: Int64 = 50_000

    /// Per-`eth_getLogs`-call block cap. Public EVM nodes reject spans
    /// over 10k blocks (`-32701 exceed maximum block range`), so the
    /// lookback is split into ≤10k sub-ranges fired in parallel.
    private static let maxLogRange: Int64 = 10_000

    /// Token contracts per `eth_getLogs` `address` array. Public nodes
    ...
    /// zkSync Era's public Blockscout instance (Etherscan-compatible,
    /// keyless). Per-chain; the EVM template overrides this constant.
    private static let blockscoutHost = "https://zksync.blockscout.com"
