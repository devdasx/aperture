...
/// **Polygon connector — an EVM sibling of `EthereumConnector`.**
///
/// A FULLY INDEPENDENT module for `.polygon`: its own `eth_getBalance`,
...
/// Endpoints come from `RPCRegistry.endpoints(for: .polygon)`.
///
/// **This is `EthereumConnector` copied for Polygon** — same code, only
/// the per-chain knobs differ. The duplication is intentional (user
/// direction): per-chain code stays isolated so a Polygon-specific
/// endpoint quirk, decimals fix, or parsing tweak never leaks into a
/// sibling EVM chain. The knobs that change from the Ethereum template:
/// - `chain` → `.polygon` (drives endpoint selection + stamps every row;
///   native units are POL, 18 decimals — identical to ETH's wei math).
/// - `blockscoutHost` → `https://polygon.blockscout.com` (Polygon's
///   public, keyless, Etherscan-compatible Blockscout instance; the
///   `txlist` row shape is byte-identical to Ethereum's).
/// The block-range knobs (`scanBlockRange` 50k, `maxLogRange` 10k,
/// `getLogsAddressChunk` 5) are unchanged: Polygon's `publicnode` node
/// caps `eth_getLogs` at 10k blocks and rejects `address` arrays over ~5
/// — the same limits the Ethereum template chunks around. Token list is
/// `EVMTokenRegistry.tokens(for: .polygon)`.
///
...
struct PolygonConnector: ChainConnector {
    let chain: SupportedChain = .polygon
    let client: RPCClient

    private static let log = Logger(subsystem: "com.thuglife.aperture", category: "polygon-connector")
    ...
    /// Native POL balance + used-address flag. Ported from
    ...
    /// every major EVM chain, Polygon included), same per-token
    ...
    /// One `eth_getLogs` call's block span. Polygon's blockscout/
    /// publicnode tolerates wide spans; the per-call cap is enforced by
    /// `maxLogRange` chunking below. Polygon's ~2s block time makes 50k
    /// blocks ≈ a day of lookback — fine for the history surface.
    private static let scanBlockRange: Int64 = 50_000

    /// Per-`eth_getLogs`-call block cap. Polygon `publicnode` rejects
    /// spans over 10k blocks, so the lookback is split into ≤10k
    /// sub-ranges fired in parallel (curl-verified 2026-06-16).
    private static let maxLogRange: Int64 = 10_000

    /// Token contracts per `eth_getLogs` `address` array. Polygon nodes
    /// block arrays above ~5 (`-32602 … blocked parameter`).
    private static let getLogsAddressChunk = 5

    /// History = native POL `txlist` (Blockscout indexer) + ERC-20
    ...
    /// Polygon's public Blockscout instance (Etherscan-compatible,
    /// keyless). Per-chain; the EVM template overrides this constant.
    /// Curl-verified 2026-06-16: the `txlist` row shape (hash / from / to
    /// / value / timeStamp / blockNumber / gasUsed / gasPrice / isError)
    /// is byte-identical to Ethereum's, so `parseNativeRows` is verbatim.
    private static let blockscoutHost = "https://polygon.blockscout.com"

    /// Native POL transfers via Blockscout's `txlist`. Paged to `limit`;
    ...
    /// the wallet. Native POL uses 18 decimals; fee = gasUsed × gasPrice
