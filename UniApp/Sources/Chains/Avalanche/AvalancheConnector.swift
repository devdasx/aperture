...
/// **Avalanche C-Chain connector — an independent EVM module for `.avalanche`.**
///
/// A FULLY INDEPENDENT module for `.avalanche` (chainId 43114 / `0xa86a`):
/// its own `eth_getBalance`, its own Multicall3 `balanceOf` token
/// batching, its own `eth_getLogs` ERC-20 `Transfer` history. It owns its
...
/// + ConcurrencyGate) — never a raw `URLSession` for the JSON-RPC path.
/// Endpoints come from `RPCRegistry.endpoints(for: .avalanche)`.
///
/// **Copied from `EthereumConnector` (the EVM reference)**, then adjusted
/// for the one Avalanche-specific quirk the existing adapters document:
/// **Avalanche has no public Blockscout instance Aperture trusts** (the
/// official explorer snowtrace.io is key-only — see
/// `EVMTransactionAdapter.blockscoutHost(for:)`, which returns `nil` for
/// `.avalanche`). So unlike Ethereum, there is no keyless `txlist`
/// indexer for native AVAX sends: native history is returned EMPTY
/// honestly (the UI's empty-state stays truthful), and ERC-20 history is
/// served purely from chunked, parallel, contract-scoped `eth_getLogs` —
/// the same path `EVMTransactionAdapter` falls back to for avalanche.
///
/// **Ported verbatim-faithful from `EVMChainAdapter` +
/// `EVMTransactionAdapter`** (same Multicall3 ABI codec, same 18-decimals
/// native AVAX, same `Transfer` topic-0, same chunked contract-scoped
/// `eth_getLogs`, same `EVMTokenRegistry.tokens(for: .avalanche)` token
/// list, same `404`/empty handling). The only re-shaping is at the
/// boundary: this connector returns the protocol's `[TokenBalance]` /
/// `[TransactionEvent]` directly instead of the adapter's raw `[Decimal?]`.
struct AvalancheConnector: ChainConnector {
    let chain: SupportedChain = .avalanche
    let client: RPCClient

    private static let log = Logger(subsystem: "com.thuglife.aperture", category: "avalanche-connector")
    ...
    /// Native AVAX balance + used-address flag. Ported from
    /// `EVMChainAdapter.fetchAccountSummary` /
    /// `fetchNativeBalance` / `fetchTransactionCount`.
    ///
    /// `eth_getBalance` returns hex-encoded wei → divided by 10^18 (AVAX,
    /// like ETH, uses 18 decimals). `eth_getTransactionCount` (nonce) > 0
    /// OR balance > 0 ⇒ "used". The nonce read is best-effort: if it fails
    /// on a fallback while the balance read succeeded, the summary is
    /// still honest (balance from the working endpoint, `isUsed` from the
    /// balance signal).
    ...
    /// ERC-20 balances for Avalanche's registry tokens
    /// (`EVMTokenRegistry.tokens(for: .avalanche)` — USDC, USDT, DAI,
    /// AUSD, EURC, FRAX, TUSD, WBTC, WETH) + the user's `customContracts`,
    /// in ONE Multicall3 `aggregate3` `eth_call` (ported from
    /// `EVMChainAdapter.fetchTokenBalancesBatched`). Multicall3 is
    /// deployed on Avalanche at the canonical address (curl-verified).
    ///
    ...
    /// every major EVM chain, Avalanche included), same per-token
    ...
    /// in input order; `nil` = that token's call reverted / returned no data.
    ...
    /// One `eth_getLogs` lookback span. ~50k blocks on Avalanche
    /// (~2s blocks ⇒ roughly the last day); the per-call cap is enforced
    /// by `maxLogRange` chunking below.
    private static let scanBlockRange: Int64 = 50_000

    /// Per-`eth_getLogs`-call block cap. publicnode/Avalanche nodes reject
    /// spans over 10k blocks (`-32701 exceed maximum block range`), so the
    /// lookback is split into ≤10k sub-ranges fired in parallel.
    ...
    /// History = native AVAX `txlist` (NONE on Avalanche — see
    /// `nativeHistoryUnavailable` below) + ERC-20 `Transfer` logs
    /// (`eth_getLogs`), merged, sorted newest-first, capped at `limit`.
    /// Ported from `EVMTransactionAdapter.fetch`, with native history
    /// suppressed because Avalanche has no keyless Blockscout `txlist`.
    ///
    ...
    /// doesn't blank the other; only a double-failure throws (native never
    /// fails here — it short-circuits to `[]`).
    ...
    // MARK: - Native history (NONE on Avalanche)

    /// **Avalanche has no keyless native-send indexer.** The official
    /// explorer snowtrace.io is key-only and there is no public
    /// Blockscout instance Aperture trusts (documented identically in
    /// `EVMTransactionAdapter.blockscoutHost(for: .avalanche) == nil`).
    /// Unlike Ethereum's `txlist` GET, there is no keyless `txlist`
    /// endpoint to page, so native AVAX history is returned EMPTY rather
    /// than fabricated — honest in code; the UI's empty-state stays
    /// truthful. ERC-20 history (the `eth_getLogs` path) is unaffected.
    private static let blockscoutHost: String? = nil

    /// Native AVAX transfers. Returns `[]` because Avalanche has no
    /// keyless `txlist` indexer (`blockscoutHost == nil`). Kept as a
    /// method (rather than inlining `[]`) so this connector stays
    /// shape-identical to its EVM siblings: a sibling chain that DOES
    /// have a Blockscout sets `blockscoutHost` and replaces this body
    /// with the paged `txlist` GET from `EthereumConnector`.
    private func fetchNativeTransactions(address: String, limit: Int) async throws -> [TransactionEvent] {
        guard Self.blockscoutHost != nil else { return [] }
        return []
    }
