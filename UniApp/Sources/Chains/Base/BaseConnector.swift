...
/// **Base connector — a fully independent EVM module for `.base`.**
///
/// A FULLY INDEPENDENT module for `.base`: its own `eth_getBalance`, its
...
/// Endpoints come from `RPCRegistry.endpoints(for: .base)`.
///
/// **This is the EVM template copied for Base** (user direction: per-chain
/// code stays isolated, even though sibling EVM chains share the JSON-RPC
/// interface). It is `EthereumConnector` with `chain = .base` and the
/// Base-specific `blockscoutHost`. Base is an OP-Stack L2: native coin is
/// ETH (18 decimals), Multicall3 is deployed at the canonical
/// `0xcA11…CA11` address, and the Base Blockscout instance is
/// Etherscan-compatible and keyless.
///
...
/// contract-scoped `eth_getLogs`, same `scanBlockRange`/`maxLogRange`/
/// `getLogsAddressChunk` knobs the adapter applies to every EVM chain).
/// The only re-shaping is at the boundary: this connector returns the
/// protocol's `[TokenBalance]` / `[TransactionEvent]` directly (resolving
/// symbol/name/decimals from `EVMTokenRegistry.tokens(for: .base)`)
/// instead of the adapter's raw `[Decimal?]`.
struct BaseConnector: ChainConnector {
    let chain: SupportedChain = .base
    let client: RPCClient

    private static let log = Logger(subsystem: "com.thuglife.aperture", category: "base-connector")
    ...
    /// every major EVM chain, Base included), same per-token sequential
    ...
    /// One `eth_getLogs` call's block span. Base is an OP-Stack L2 with
    /// ~2s blocks, so 50k blocks ≈ ~28h of lookback; the per-call cap is
    /// enforced by `maxLogRange` chunking below. Matches the EVM adapter's
    /// uniform `scanBlockRange` (the adapter applies one value across all
    /// EVM chains).
    private static let scanBlockRange: Int64 = 50_000

    /// Per-`eth_getLogs`-call block cap. publicnode/L2 nodes reject spans
    ...
    /// Base's public Blockscout instance (Etherscan-compatible, keyless).
    /// Per-chain; the EVM template overrides this constant — Ethereum uses
    /// `eth.blockscout.com`, Base uses `base.blockscout.com`. Matches
    /// `EVMTransactionAdapter.blockscoutHost(for: .base)`.
    private static let blockscoutHost = "https://base.blockscout.com"
