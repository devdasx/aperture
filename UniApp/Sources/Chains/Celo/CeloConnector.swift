...
/// **Celo connector — an EVM-fleet sibling of `EthereumConnector`.**
///
/// A FULLY INDEPENDENT module for `.celo`: its own `eth_getBalance`, its
/// own Multicall3 `balanceOf` token batching, its own `eth_getLogs`
/// token-transfer history + Blockscout `txlist` native history. It owns
/// its request shapes + parsing end-to-end and dispatches every call
/// through the shared `RPCClient` actor (rotation + rate-limit +
/// circuit-breaking + ConcurrencyGate) — never a raw `URLSession`.
/// Endpoints come from `RPCRegistry.endpoints(for: .celo)`
/// (publicnode / forno / drpc / keyed 1rpc, chainId `0xa4ec` = 42220).
///
/// **This is the EthereumConnector template, copied not shared** (user
/// direction): the only per-chain divergences are `chain = .celo` and
/// `blockscoutHost = "https://celo.blockscout.com"`. Celo's curated
/// tokens (USDC `0xcebA…118C`, USDT `0x4806…3D5e`, both 6-decimal) come
/// from `EVMTokenRegistry.tokens(for: .celo)`; the same 18-decimal
/// native unit (CELO), the same Multicall3 ABI codec, and the same
/// chunked contract-scoped `eth_getLogs` apply because Celo is a
/// standard EVM chain. Keeping a separate file means a future
/// Celo-only quirk (its historical fee-currency field, a node-specific
/// log-range cap) is edited here without touching any sibling.
///
/// **Ported verbatim-faithful from `EVMChainAdapter` +
/// `EVMTransactionAdapter`** (same endpoints, same parsing, same
/// 18-decimals native, same Multicall3 selector/address, same ≤10k
/// `eth_getLogs` chunking, same Blockscout `txlist`). The only
/// re-shaping is at the boundary: this connector returns the protocol's
/// `[TokenBalance]` / `[TransactionEvent]` directly (resolving
/// symbol/name/decimals from `EVMTokenRegistry`) instead of the
/// adapter's raw `[Decimal?]`.
struct CeloConnector: ChainConnector {
    let chain: SupportedChain = .celo
    let client: RPCClient

    private static let log = Logger(subsystem: "com.thuglife.aperture", category: "celo-connector")
    ...
    /// Native CELO balance + used-address flag. Ported from
    /// `EVMChainAdapter.fetchAccountSummary` /
    /// `fetchNativeBalance` / `fetchTransactionCount`.
    ///
    /// `eth_getBalance` returns hex-encoded wei → divided by 10^18 (CELO
    /// uses the standard 18-decimal native unit).
    ...
    /// Multicall3 is deployed on Celo at the canonical address
    /// (live-verified 2026-06-16), so the batched path is the norm.
    ///
    ...
    /// Celo and every major EVM chain), same per-token sequential
    /// fallback when Multicall3 isn't deployed (empty bytes /
    /// deterministic JSON-RPC error). Returns one `Decimal?` per contract
    /// in input order; `nil` = that token's call reverted / no data.
    ...
    /// One `eth_getLogs` call's lookback span. The per-call cap is
    /// enforced by `maxLogRange` chunking below. Celo's block time is
    /// ~1s, so 50k blocks is a shorter wall-clock window than on
    /// Ethereum — Blockscout `tokentx`/`txlist` is the primary source;
    /// this is the keyless-node fallback.
    private static let scanBlockRange: Int64 = 50_000

    /// Per-`eth_getLogs`-call block cap. publicnode / forno / drpc Celo
    /// nodes reject spans over 10k blocks (`-32701 exceed maximum block
    /// range`), so the lookback is split into ≤10k sub-ranges fired in
    /// parallel.
    ...
    /// History = native CELO `txlist` (Blockscout indexer) + ERC-20
    ...
    /// Celo's public Blockscout instance (Etherscan-compatible,
    /// keyless). Per-chain; this is the one EthereumConnector constant
    /// that diverges for Celo (live-verified 2026-06-16).
    private static let blockscoutHost = "https://celo.blockscout.com"

    /// Native CELO transfers via Blockscout's `txlist`. Paged to `limit`;
    ...
    /// the wallet. Native CELO uses 18 decimals; fee = gasUsed × gasPrice
