...
/// **Optimism connector — an independent EVM module for `.optimism`.**
///
/// A FULLY INDEPENDENT module for `.optimism`: its own
...
/// `RPCRegistry.endpoints(for: .optimism)`.
///
/// **This is the Ethereum reference copied per-chain (user direction).**
/// The duplication is intentional: Optimism's endpoint quirks stay
/// isolated here and never leak into a sibling EVM connector. Optimism
/// is an OP-Stack L2 — native coin is **ETH (18 decimals)**, the
/// canonical Multicall3 is deployed at the same address as every major
/// EVM chain, and its public Blockscout instance speaks the same
/// Etherscan-compatible `txlist` API. The only per-chain knobs that
/// differ from `EthereumConnector` are `chain` and `blockscoutHost`.
///
/// **Optimism block cadence note.** OP blocks land roughly every 2s
/// (vs Ethereum's ~12s), so the shared `scanBlockRange` of 50k blocks
/// covers ~1 day of history here rather than ~7 days. The value is kept
/// identical to the EVM reference on purpose: the per-call `maxLogRange`
/// chunking and `getLogsAddressChunk` limits are what the upstreams
/// actually enforce, and widening the lookback per-chain is a tuning
/// decision the Integrate phase owns, not the connector.
///
...
struct OptimismConnector: ChainConnector {
    let chain: SupportedChain = .optimism
    let client: RPCClient

    private static let log = Logger(subsystem: "com.thuglife.aperture", category: "optimism-connector")
    ...
    /// every major EVM chain — verified live-deployed on Optimism), same
    ...
    /// One `eth_getLogs` call's block span. Identical to the EVM
    /// reference; the per-call cap is enforced by `maxLogRange` chunking
    /// below. On Optimism (~2s blocks) 50k blocks ≈ ~1 day of history.
    private static let scanBlockRange: Int64 = 50_000

    /// Per-`eth_getLogs`-call block cap. OP publicnode rejects spans that
    /// would exceed its result cap, so the lookback is split into ≤10k
    ...
    /// Optimism's public Blockscout instance (Etherscan-compatible,
    /// keyless). Per-chain; this is the only token-/balance-affecting
    /// constant that differs from `EthereumConnector`.
    ///
    /// NOTE (verified live): this host currently 301-redirects to
    /// `https://explorer.optimism.io/api?…`, which returns the same
    /// Etherscan-compatible `{message,result,status}` envelope. The
    /// native-history GET below uses `URLSession.shared`, which follows
    /// 3xx redirects automatically, so the request transparently lands
    /// on the canonical upstream and parses identically.
    private static let blockscoutHost = "https://optimism.blockscout.com"
