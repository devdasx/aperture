...
/// **Scroll connector — an independent EVM module.**
///
/// A FULLY INDEPENDENT module for `.scroll`: its own `eth_getBalance`,
...
/// JSON-RPC. Endpoints come from `RPCRegistry.endpoints(for: .scroll)`.
///
/// **This is `EthereumConnector` with the chain swapped.** It is a
/// verbatim-faithful copy of the EVM reference (same Multicall3 ABI
/// codec, same chunked contract-scoped `eth_getLogs`, same 18-decimals
/// native). The duplication is intentional (user direction): Scroll's
/// endpoint quirks stay isolated from every sibling EVM chain. The only
/// per-chain knobs that differ from Ethereum are `chain` (`.scroll`),
/// `blockscoutHost` (`https://scroll.blockscout.com`, which 301-redirects
/// to the Etherscan-compatible `scrollscan.com/api` — `URLSession`
/// follows it transparently), and the logger category. The block-range
/// constants match the EVM template's uniform values, identical to what
/// `EVMTransactionAdapter` uses for Scroll today.
///
/// **Ported verbatim-faithful from `EVMChainAdapter` +
/// `EVMTransactionAdapter`.** The only re-shaping is at the boundary:
/// this connector returns the protocol's `[TokenBalance]` /
/// `[TransactionEvent]` directly (resolving symbol/name/decimals from
/// `EVMTokenRegistry.tokens(for: .scroll)`) instead of the adapter's raw
/// `[Decimal?]`.
struct ScrollConnector: ChainConnector {
    let chain: SupportedChain = .scroll
    let client: RPCClient

    private static let log = Logger(subsystem: "com.thuglife.aperture", category: "scroll-connector")
    ...
    ///
    /// Scroll is an Ethereum L2 whose native gas token is ETH (18
    /// decimals). `eth_getBalance` returns hex-encoded wei → divided by
    /// 10^18. `eth_getTransactionCount` (nonce) > 0 OR balance > 0 ⇒
    /// "used". The nonce read is best-effort: if it fails on a fallback
    /// while the balance read succeeded, the summary is still honest
    /// (balance from the working endpoint, `isUsed` from the balance
    /// signal).
    ...
    /// Multicall3 is deployed on Scroll at the canonical address
    /// (curl-verified: `aggregate3` decodes both registry tokens).
    ///
    ...
    /// every major EVM chain, Scroll included), same per-token sequential
    /// fallback when Multicall3 isn't deployed (empty bytes / deterministic
    /// JSON-RPC error). Returns one `Decimal?` per contract in input
    /// order; `nil` = that token's call reverted / returned no data.
    ...
    /// by `maxLogRange` chunking below. Matches the EVM template's
    /// uniform 50k value (identical to `EVMTransactionAdapter` for Scroll).
    private static let scanBlockRange: Int64 = 50_000

    /// Per-`eth_getLogs`-call block cap. publicnode/Scroll nodes reject
    ...
    /// Scroll's public Blockscout instance (Etherscan-compatible,
    /// keyless). It 301-redirects `/api` to `scrollscan.com/api`, which
    /// `URLSession.shared` follows transparently (curl-verified: the
    /// redirect target returns the standard `{result:[…]}` envelope).
    /// Per-chain; the EVM template overrides this constant.
    private static let blockscoutHost = "https://scroll.blockscout.com"
