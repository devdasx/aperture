...
/// **Arbitrum connector — an independent EVM module for `.arbitrum`.**
///
/// A FULLY INDEPENDENT copy of the EVM template for Arbitrum One: its
/// own `eth_getBalance`, its own Multicall3 `balanceOf` token batching,
/// its own `eth_getLogs` token-transfer history + Blockscout `txlist`
...
/// `URLSession` for JSON-RPC. Endpoints come from
/// `RPCRegistry.endpoints(for: .arbitrum)`.
///
/// **This is `EthereumConnector` with the Arbitrum knobs swapped** (user
/// direction: per-chain code stays isolated, the duplication is
/// intentional). The only differences from the Ethereum reference are
/// the chain-specific constants:
/// - `chain = .arbitrum` (drives endpoint selection + row stamping)
/// - `blockscoutHost = "https://arbitrum.blockscout.com"`
///   (Arbitrum One's public Blockscout indexer — same `txlist` shape as
///   eth.blockscout.com; verified live against the canonical fleet
///   address)
/// - `scanBlockRange` / `maxLogRange` / `getLogsAddressChunk` mirror the
///   shared `EVMTransactionAdapter` knobs (50k lookback, ≤10k per
///   `eth_getLogs`, ≤5 contracts per `address` array) — Arbitrum's
///   publicnode/tenderly nodes accept these spans; `arb-drpc` is
///   balance-only in the registry (malformed getLogs), the rotation
///   handles that.
///
/// **Ported verbatim-faithful from `EVMChainAdapter` +
/// `EVMTransactionAdapter`** (same endpoints, same parsing, same
/// 18-decimals native, same Multicall3 ABI codec `0x82ad56cb` at
/// `0xcA11…CA11`, same chunked contract-scoped `eth_getLogs`). Token
/// metadata resolves from `EVMTokenRegistry.tokens(for: .arbitrum)`
/// (USDC, USDT, DAI, USD0, USDai, USDe, WBTC, WETH). The only re-shaping
/// is at the boundary: this connector returns the protocol's
/// `[TokenBalance]` / `[TransactionEvent]` directly instead of the
/// adapter's raw `[Decimal?]`.
struct ArbitrumConnector: ChainConnector {
    let chain: SupportedChain = .arbitrum
    let client: RPCClient

    private static let log = Logger(subsystem: "com.thuglife.aperture", category: "arbitrum-connector")
    ...
    /// (Arbitrum's native gas token is ETH, 18 decimals).
    ...
    /// ERC-20 balances for Arbitrum's registry tokens + the user's
    ...
    /// every major EVM chain, Arbitrum included — live-verified), same
    /// per-token sequential fallback when Multicall3 isn't deployed
    /// (empty bytes / deterministic JSON-RPC error). Returns one
    /// `Decimal?` per contract in input order; `nil` = that token's call
    /// reverted / returned no data.
    ...
    /// One `eth_getLogs` call's block span. Arbitrum's publicnode/
    /// tenderly tolerate wide spans; the per-call cap is enforced by
    /// `maxLogRange` chunking below. Mirrors `EVMTransactionAdapter`'s
    /// shared 50k lookback (Arbitrum blocks are fast, so this is a
    /// shorter wall-clock window than on Ethereum — acceptable for a
    /// recent-history smoke view; the full-history path stays in the
    /// indexer `txlist`).
    private static let scanBlockRange: Int64 = 50_000

    /// Per-`eth_getLogs`-call block cap. publicnode/Arbitrum nodes reject
    /// spans over 10k blocks, so the lookback is split into ≤10k
    /// sub-ranges fired in parallel.
    ...
    /// History = native ETH `txlist` (Arbitrum Blockscout indexer) +
    ...
    /// Arbitrum One's public Blockscout instance (Etherscan-compatible,
    /// keyless). Per-chain constant — the EVM template overrides this.
    private static let blockscoutHost = "https://arbitrum.blockscout.com"
