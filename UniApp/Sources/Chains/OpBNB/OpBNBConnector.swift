...
/// **opBNB connector — an independent EVM module (copied from `EthereumConnector`).**
///
/// A FULLY INDEPENDENT module for `.opBNB`: its own `eth_getBalance`,
/// its own Multicall3 `balanceOf` token batching, its own `eth_getLogs`
/// token-transfer history. It owns its request shapes + parsing
/// end-to-end and dispatches every call through the shared `RPCClient`
/// actor (rotation + rate-limit + circuit-breaking + ConcurrencyGate) —
/// never a raw `URLSession` for JSON-RPC. Endpoints come from
/// `RPCRegistry.endpoints(for: .opBNB)`.
///
/// **Copied from the `EthereumConnector` EVM template** (user direction:
/// per-chain code stays isolated — one chain's quirk never leaks into a
/// sibling). The structure is identical to Ethereum; only the per-chain
/// knobs differ, and they are the ONLY intentional divergences:
///
/// - `chain = .opBNB` — drives `RPCRegistry.endpoints(for:)`,
///   `EVMTokenRegistry.tokens(for:)` (opBNB ships one token: USDT,
///   18 decimals — `0x9e5AAC1Ba1a2e6aEd6b32689DFcF62A509Ca96f3`), and the
///   `ticker` stamped on native history rows (opBNB's native coin is BNB).
/// - `blockscoutHost = nil` — opBNB has NO public, keyless Blockscout /
///   Etherscan-compatible indexer we trust (its official explorer
///   `opbnb.bscscan.com` is key-gated). This mirrors `EVMTransactionAdapter`
///   exactly, where `.opBNB` (with `.bnbChain` and `.avalanche`) returns
///   nil from `blockscoutHost(for:)`. The honest consequence: NATIVE BNB
///   send history is empty (native transfers emit no logs, so `eth_getLogs`
///   can't see them and there's no indexer to ask), while ERC-20 token
///   history still works via the chunked contract-scoped `eth_getLogs`
///   path. The UI's empty-state stays truthful (Rule #16).
/// - `chainId 0xcc` (204) — confirmed live so the endpoint pool is opBNB.
///
/// Everything else (Multicall3 ABI codec, the ≤10k-block getLogs chunking,
/// the ≤5-contract address chunking, the 18-decimals native scaling, the
/// hex helpers) is byte-for-byte the Ethereum reference, verified live
/// against `opbnb-rpc.publicnode.com` on 2026-06-16 (Multicall3 deployed
/// at `0xcA11…CA11`, `aggregate3` decodes cleanly, contract-scoped
/// `eth_getLogs` returns real `Transfer` rows).
struct OpBNBConnector: ChainConnector {
    let chain: SupportedChain = .opBNB
    let client: RPCClient

    private static let log = Logger(subsystem: "com.thuglife.aperture", category: "opbnb-connector")
    ...
    /// every major EVM chain — verified live on opBNB at
    /// `0xcA11bde05977b3631167028862bE2a173976CA11`), same per-token
    /// sequential fallback when Multicall3 isn't deployed (empty bytes /
    /// deterministic JSON-RPC error). Returns one `Decimal?` per contract
    /// in input order; `nil` = that token's call reverted / returned no
    /// data.
    ...
    /// One `eth_getLogs` call's block span. The per-call cap is enforced
    /// by `maxLogRange` chunking below. opBNB mints blocks ~1s apart
    /// (much faster than Ethereum), so 50k blocks ≈ ~14h of activity —
    /// the same constant the EVM template uses; the chunking below keeps
    /// every individual call inside the node's hard limit regardless.
    private static let scanBlockRange: Int64 = 50_000

    /// Per-`eth_getLogs`-call block cap. opBNB's publicnode rejects spans
    /// over 10k blocks (`-32701 exceed maximum block range`, verified
    /// against the Ethereum-family limit), so the lookback is split into
    /// ≤10k sub-ranges fired in parallel.
    ...
    /// History = native BNB `txlist` (Blockscout indexer — opBNB has
    /// none, see `blockscoutHost`, so this resolves to `[]` honestly) +
    /// ERC-20 `Transfer` logs (`eth_getLogs`), merged, sorted
    /// newest-first, capped at `limit`. Ported from
    /// `EVMTransactionAdapter.fetch`.
    ///
    ...
    /// doesn't blank the other; only a double-failure throws. On opBNB the
    /// native direction is structurally empty (no indexer), so in practice
    /// a successful token scan always yields a non-throwing result.
    ...
    /// opBNB has NO public, keyless Blockscout / Etherscan-compatible
    /// indexer we trust (official explorer `opbnb.bscscan.com` is
    /// key-gated). Mirrors `EVMTransactionAdapter.blockscoutHost(for:)`,
    /// which returns nil for `.opBNB` (alongside `.bnbChain` /
    /// `.avalanche`). `nil` ⇒ native send history is empty (native BNB
    /// transfers emit no logs; `eth_getLogs` can't see them and there's
    /// no indexer to ask). Honest in code; the UI's empty-state stays
    /// truthful (Rule #16).
    private static let blockscoutHost: String? = nil

    /// Native BNB transfers via Blockscout's `txlist`. opBNB has no
    /// Blockscout host (`blockscoutHost == nil`) so this returns `[]`
    /// immediately — the honest answer, not a stub. Kept structurally
    /// identical to the EVM template so a future opBNB indexer is a
    /// one-line `blockscoutHost` change. Ported from
    ...
    private func fetchNativeTransactions(address: String, limit: Int) async throws -> [TransactionEvent] {
        guard let host = Self.blockscoutHost else { return [] }
    ...
    /// the wallet. Native BNB uses 18 decimals; fee = gasUsed × gasPrice
