...
actor RPCClient {
    ...
    /// POST a **raw** body (not a JSON dict) to `chain`'s REST endpoints —
    /// for broadcast paths whose body is raw transaction hex (Esplora
    /// `/tx`, Haskoin `/transactions`), form-encoded XDR (Stellar
    /// `/transactions`), or any non-JSON payload. Same fallback rotation +
    /// circuit breaker as `callRESTPost`. On a non-2xx the server's body is
    /// folded into the thrown error so a broadcast rejection surfaces its
    /// real reason (Rule #16 / Rule #26) instead of a bare status code.
        ...
        for endpoint in endpoints where endpoint.kind == .rest {
            ...
            do {
                ...
                let data = try await dispatchRESTPostRaw(
                    endpoint: endpoint, path: path, body: body, contentType: contentType
                )
                recordSuccess(for: endpoint.id)
                return data
            } catch {
                if case .cancelled = error { throw error }
                if case .rateLimited = error { lastError = error; continue }
                lastError = error
                recordFailure(for: endpoint.id)
                continue
            }
    ...
    private func dispatchRESTPostRaw(
        ...
        let url = path.isEmpty ? endpoint.url : endpoint.url.appendingPathComponent(path)
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        ...
        if let http = response as? HTTPURLResponse {
            ...
            if !(200..<300).contains(http.statusCode) {
                ...
                throw .invalidResponse("HTTP \(http.statusCode) from \(endpoint.id): \(snippet)")
            }
        }
        return responseData
    }
