...
struct RealRPCBalanceScanner: BalanceScanner {
    ...
    func scan(
        ...
        // Data fetching is disabled for some chains (2026-06-21 user
        // direction — EVM + Bitcoin family + Tron): drop those addresses up
        // front so no balance/token RPC is ever fired, whatever the caller.
    ...
    func streamScan(
        ...
        // Data fetching is disabled for some chains (2026-06-21 — EVM +
        // Bitcoin family + Tron): drop those addresses (and their custom
        // tokens) so the stream never scans them — every caller skips them
        // through this one point.
        let addresses = addresses.filter { !$0.key.fetchingDisabled }
        let customTokens = customTokens.filter { !$0.key.fetchingDisabled }
