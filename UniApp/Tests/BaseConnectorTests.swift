...
/// **Base connector live-RPC smoke test.**
///
/// The EVM-template test copied for Base (swap the connector type +
/// address). Hits live public RPC through `RPCClient.shared` (rotation +
/// rate-limit + circuit-breaking apply), so it proves `BaseConnector`'s
/// request shapes + parsing work against the real Base upstreams.
///
...
/// state is verifiable in any block explorer (basescan.org /
/// base.blockscout.com).
struct BaseConnectorTests {

    let connector = BaseConnector()
    /// Real, publicly-known address — the canonical fleet test address.
    let address = "0xB61d7338a20Fa652EC9eD20d1392D33644837c8a"

    @Test("Base native balance read succeeds and is non-negative")
    ...
    @Test("Base history returns an array without throwing")
    ...
    @Test("Base token balances return an array")
