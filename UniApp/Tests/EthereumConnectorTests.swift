...
/// **Worked example — Ethereum connector live-RPC smoke test.**
///
/// The EVM-template test every EVM-chain connector test copies (swap
/// the connector type + address). Hits live public RPC through
/// `RPCClient.shared` (rotation + rate-limit + circuit-breaking apply),
/// so it proves `EthereumConnector`'s request shapes + parsing work
/// against the real upstreams.
///
/// **Address.** `0xB61d7338a20Fa652EC9eD20d1392D33644837c8a` — a real,
/// publicly-known Ethereum address (provided as the canonical test
/// address for this fleet). On-chain state is verifiable in any block
/// explorer (etherscan.io / eth.blockscout.com).
struct EthereumConnectorTests {

    let connector = EthereumConnector()
    /// Real, publicly-known Ethereum address — the canonical fleet test address.
    let address = "0xB61d7338a20Fa652EC9eD20d1392D33644837c8a"

    @Test("Ethereum native balance read succeeds and is non-negative")
    ...
    @Test("Ethereum history returns an array without throwing")
    ...
    @Test("Ethereum token balances return an array")
