...
/// **Arbitrum connector live-RPC smoke test.**
///
/// The EVM-template test (copied from `EthereumConnectorTests`, swapping
/// the connector type + chain). Hits live public RPC through
/// `RPCClient.shared` (rotation + rate-limit + circuit-breaking apply),
/// so it proves `ArbitrumConnector`'s request shapes + parsing work
/// against the real Arbitrum One upstreams (publicnode / tenderly /
...
/// state is verifiable in any block explorer (arbiscan.io /
/// arbitrum.blockscout.com).
struct ArbitrumConnectorTests {

    let connector = ArbitrumConnector()
    /// Real, publicly-known address — the canonical fleet test address.
    let address = "0xB61d7338a20Fa652EC9eD20d1392D33644837c8a"

    @Test("Arbitrum native balance read succeeds and is non-negative")
    ...
    @Test("Arbitrum history returns an array without throwing")
    ...
    @Test("Arbitrum token balances return an array")
