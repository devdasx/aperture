...
/// Why the user just landed on home after adding a wallet — drives the
/// first-refresh skeleton variant (empty-state vs balance-card).
enum WalletFirstRefreshKind: String, Sendable {
    /// Brand-new empty wallet — show empty-state skeleton (not balance card).
    case created
    /// Imported wallet may already hold funds — show balance-card skeleton.
    case imported
}
...
enum WalletFirstRefreshPresentationCenter {
    ...
    /// Legacy key from the removed completion sheet; still cleared on finish.
    static let completionDismissedAtKey = "pendingWalletFirstRefreshCompletionDismissedAt"
    static let kindKey = "pendingWalletFirstRefreshKind"

    static func markNewWallet(_ walletId: UUID?, kind: WalletFirstRefreshKind) {
        ...
        store.set(kind.rawValue, forKey: kindKey)
    }

    static func kind(for walletId: UUID) -> WalletFirstRefreshKind {
        let store = AppPreferenceStore.shared
        guard store.string(walletIdKey, default: "") == walletId.uuidString else {
            return .imported
        }
        let raw = store.string(kindKey, default: WalletFirstRefreshKind.imported.rawValue)
        return WalletFirstRefreshKind(rawValue: raw) ?? .imported
    }
