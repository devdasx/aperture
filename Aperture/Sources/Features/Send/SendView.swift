...
struct SendView: View {
        ...
        NavigationStack(path: $navigationPath) {
            ...
            .navigationDestination(for: SendDestination.self) { destination in
                ...
                case let .review(draft):
                    // P2-009: never invent a random walletId for signing context.
                    if let wallet = activeWallet {
                        SendReviewLoader(
                            draft: draft,
                            walletId: wallet.id,
                            walletHasPassphrase: wallet.hasPassphrase,
                            onClose: closeFlow
                        )
                    } else {
                        ContentUnavailableView(
                            ...
                            description: Text("Select a wallet before sending.")
                        )
                    }
                }
            }
    ...
    /// P2-007: match Receive — prefer `isReceivePreferred` so send from
    /// the same address the user shows as QR.
