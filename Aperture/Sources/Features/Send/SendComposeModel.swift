...
final class SendComposeModel {
    ...
    /// Engage send-all for the **single** amount field.
    ///
    /// P3-018: Max + multi-recipient is unsupported (signing refuses).
    /// When there is more than one amount row, this is a no-op so the
    /// UI never arms `isMaxSend` for multi drafts.
    ///
    ...
    func engageMax() {
        ...
        let crypto: Decimal = {
            ...
            // Fee not loaded yet — seed with available; `setBalances` /
        ...
        case .fiat:
            if let price = assetUnitPrice, price > 0 {
                amounts[0].amountText = Self.plainString(crypto * price, decimals: 2)
            } else {
                entryUnit = .crypto
                amounts[0].amountText = Self.plainString(crypto, decimals: effectiveDecimals)
            }
    ...
    func loadUTXOs(
        ...
        do {
            // Live fetch first — never short-circuit on stale cache (P0).
            // Source matches balance scanners via UTXOService.
            ...
            // Last-good: balance scanner already wrote Electrum/REST UTXOs
            // into chain_utxos — same store of truth, just not live.
            if let walletId,
