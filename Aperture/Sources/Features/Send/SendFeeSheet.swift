...
struct SendFeeSheet: View {
        ...
        if let quote {
            List {
                if quote.hasSpeedTiers {
                    // M-011 / P2-019: ETA copy is for **this network**, not universal.
                    let eta = Self.etaCopy(for: model.chain)
                    Section {
                        tierRow(.slow, quote: quote, label: "Slower", time: LocalizedStringKey(eta.slow))
                        tierRow(.normal, quote: quote, label: "Normal", time: LocalizedStringKey(eta.normal))
                        tierRow(.fast, quote: quote, label: "Faster", time: LocalizedStringKey(eta.fast))
                    } header: {
                        UniCaption(text: "Speed", color: UniColors.Text.tertiary)
                    } footer: {
                        Text(verbatim: String(
                            format: String.apertureLocalized(
                                "Rough confirmation times for %@. Not the same on every network."
                            ),
                            model.chain.displayName
                        ))
    ...
    private func customFields(_ quote: FeeQuote) -> some View {
        ...
        case .solana:
            // Solana's user lever is the priority compute-unit price in
            // micro-lamports/CU (solana.com/docs/core/fees).
            customField("Priority price", text: $customSolPriority, suffix: "µ-lamports/CU")
        case .stellarPerOp:
        ...
        default:
            ...
            // section never renders for them) — keep the preset tiers honest.
            EmptyView()
        }
    }
    ...
    /// M-011 / P2-019: rough confirmation ETA for **this chain only**
    /// (not universal block times across all networks).
    nonisolated static func etaCopy(for chain: SupportedChain) -> (slow: String, normal: String, fast: String) {
        switch chain {
        case .bitcoin, .litecoin, .bitcoinCash, .dogecoin:
            return ("~30–60 min", "~10–30 min", "~ next block (~10 min)")
        case .ethereum:
            return ("~5–15 min", "~1–3 min", "~ next block (~12 s)")
        case .solana:
            return ("~30–60 s", "~ a few s", "~ next slot (~0.4 s)")
        case .ripple:
            return ("~10–20 s", "~3–5 s", "~ next ledger (~3–5 s)")
        case .tron:
            return ("~1–3 min", "~15–60 s", "~ next block (~3 s)")
        case .ton:
            return ("~30–60 s", "~ a few s", "~ next block")
        case .near, .aptos, .sui:
            return ("~30–60 s", "~ a few s", "~ next block")
        case .polkadot:
            return ("~1–2 min", "~12–24 s", "~ next block (~6 s)")
        case .stellar:
            return ("~30–60 s", "~5–10 s", "~ next ledger (~5 s)")
        default:
            // Other EVM L2s / sidechains: sub-minute typical.
            if chain.family == .evm {
                return ("~1–3 min", "~15–60 s", "~ next block")
            }
            return ("~ slower", "~ normal", "~ faster")
        }
    }
    ...
    private func seedCustomFields(_ quote: FeeQuote) {
        ...
        if customSolPriority.isEmpty, let p = normal.computeUnitPrice {
            customSolPriority = SendComposeModel.plainString(p, decimals: 0)
        }
    ...
    private func applyCustomIfNeeded() {
        ...
        custom.computeUnitPrice = base.computeUnitPrice
        custom.computeUnitLimit = base.computeUnitLimit
        ...
        case .evmLegacy:
            ...
            custom = ComposeFeeService.recomputeEVMTotals(custom, decimals: dec)
        case .solana:
            // Custom priority compute-unit price (µ-lamports/CU). 0 is valid
            // (no priority) so allow ≥ 0; require a parseable field.
            guard let price = SendComposeModel.parseAmount(customSolPriority), price >= 0 else { return }
            custom.computeUnitPrice = price
            custom = ComposeFeeService.recomputeSolanaTotals(custom, decimals: dec)
        case .stellarPerOp:
            // Custom per-operation base-fee bid in stroops (clamp ≥ 100/op).
