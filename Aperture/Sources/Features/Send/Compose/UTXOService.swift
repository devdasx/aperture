...
struct UTXOService: Sendable {
    ...
    func fetchUTXOs(addresses: [String], chain: SupportedChain) async throws -> [SelectedUTXO] {
        ...
        let trace = await DiagnosticsAPIMonitor.shared.begin(
            ...
            metadata: [
                ...
                "addressCount": "\(unique.count)",
                "address": Self.abbreviated(unique[0]),
                "source": Self.sourceLabel(for: chain)
            ]
        )
        do {
            ...
            case .litecoin:
                utxos = try await fetchEsploraMany(addresses: unique, chain: chain)
            case .dogecoin:
                utxos = try await fetchDogecoinMany(addresses: unique)
            default:
    ...
    ///
    /// Source matches the balance scanner for each chain (Electrum batch for
    /// BTC/BCH; REST multi-address for LTC/DOGE).
    ///
    ...
    /// Esplora `/address/{addr}/utxo` (LTC — same REST as balance scanner).
    /// Script not returned; the signer derives it for own addresses.
    ...
    private func fetchEsploraMany(addresses: [String], chain: SupportedChain) async throws -> [SelectedUTXO] {
        if addresses.count == 1, let only = addresses.first {
            return try await fetchEsplora(address: only, chain: chain)
        }
        return try await withThrowingTaskGroup(of: [SelectedUTXO].self) { group in
            for address in addresses {
                group.addTask {
                    try await self.fetchEsplora(address: address, chain: chain)
                }
            }
    ...
    private func fetchDogecoinMany(addresses: [String]) async throws -> [SelectedUTXO] {
        if addresses.count == 1, let only = addresses.first {
            return try await DogecoinDataClient.shared.fetchUTXOs(address: only)
        }
        return try await withThrowingTaskGroup(of: [SelectedUTXO].self) { group in
            for address in addresses {
                group.addTask {
                    try await DogecoinDataClient.shared.fetchUTXOs(address: address)
                }
            }
            var all: [SelectedUTXO] = []
            for try await utxos in group {
