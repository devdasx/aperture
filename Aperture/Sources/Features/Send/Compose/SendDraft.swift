...
struct SendRecipientAmount: Codable, Hashable, Sendable, Identifiable {
    ...
    let amount: Decimal
    let name: String?
    /// Stable row id. P2-026: never use address alone — duplicate
    /// destinations must not share SwiftUI identity.
    let rowId: UUID

    var id: String { rowId.uuidString }

    init(address: String, amount: Decimal, name: String? = nil, rowId: UUID = UUID()) {
