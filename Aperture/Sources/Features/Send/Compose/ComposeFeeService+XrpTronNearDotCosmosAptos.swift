...
extension ComposeFeeService {
    ...
    /// P2-010: ABI-encode the 20-byte address (not raw base58 T-address hex-pad).
    private func estimateTronEnergy(owner: String, contract: String, to: String) async throws -> Decimal {
        guard let toParam = Self.tronABIAddressParameter(to) else {
            // Cannot encode destination — honest fallback, not a fake T-string pad.
            return 65000
        }
        // amount = 1 (minimal) left-padded to 32 bytes.
        let amountParam = String(repeating: "0", count: 63) + "1"
        let param = toParam + amountParam
        ...
        if let used = (root["energy_used"] as? NSNumber)?.decimalValue, used > 0 { return used }
        return 65000
    }

    /// TRC-20 ABI `address` word: 32-byte left-padded hex of the 20-byte
    /// account id. Accepts base58check `T…` or hex `41…` / `0x…`.
    static func tronABIAddressParameter(_ address: String) -> String? {
        let trimmed = address.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return nil }

        let accountHex20: String
        if trimmed.hasPrefix("T") {
            // Base58Check → 21-byte payload (0x41 + 20) + 4 checksum.
            guard let decoded = Base58.decodeBytes(trimmed), decoded.count >= 25 else { return nil }
            let payload = Array(decoded.prefix(21))
            guard payload.count == 21, payload[0] == 0x41 else { return nil }
            accountHex20 = payload.dropFirst().map { String(format: "%02x", $0) }.joined()
        } else {
            var hex = trimmed.hasPrefix("0x") || trimmed.hasPrefix("0X")
                ? String(trimmed.dropFirst(2))
                : trimmed
            hex = hex.lowercased()
            if hex.hasPrefix("41"), hex.count == 42 {
                hex = String(hex.dropFirst(2))
            }
            guard hex.count == 40, hex.allSatisfy(\.isHexDigit) else { return nil }
            accountHex20 = hex
        }
        return String(repeating: "0", count: 24) + accountHex20
    }
    ...
    func polkadotQuote(_ ctx: Context) async throws -> FeeQuote {
        ...
        // P2-004: multi-batch utility.batch scales weight ≈ linearly with N
        // keepAlive calls — under-reserving inclusion fee for multi-send.
        let recipientCount = max(1, ctx.recipientCount)
        let singlePartialFeePlancks = Decimal(152_000_000) // ~0.0152 DOT per transfer
        let partialFeePlancks = singlePartialFeePlancks * Decimal(recipientCount)
