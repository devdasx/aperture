...
struct WalletHomeMinValueView: View {
    ...
    private func commitCustom() {
        ...
        // P2-027: locale-aware Decimal parse (not Double(trimmed)).
        if let parsed = Self.parseFiatThreshold(trimmed), parsed >= 0 {
            // Threshold preference is Double for GRDB; round-trip via Decimal
            // so locale separators don't invent values.
            thresholdRaw = NSDecimalNumber(decimal: parsed).doubleValue
        }
    }

    /// Parse a fiat threshold from user text using locale decimal separator.
    nonisolated static func parseFiatThreshold(_ raw: String) -> Decimal? {
        ...
        // Fallback: en_US_POSIX + plain Decimal (accepts "1.5" always).
