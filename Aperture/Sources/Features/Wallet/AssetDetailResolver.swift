...
enum AssetDetailResolver {
    ...
    private static func resolveNative(
        ...
        // Sum every native balance row on this chain (Solana Phantom + Trust
        // each have their own token_balances row — match home portfolio).
        var amount: Decimal = .zero
        var fiatSum: Decimal = .zero
        var hasFiat = false
        var fiatCode = fallbackCurrencyCode
        for entry in heldRows where entry.chain == chain
            ...
            && entry.balance.tokenSymbol == chain.ticker
        {
            ...
            if entry.balance.fiatValueCached > 0 {
                fiatSum += entry.balance.fiatValueCached
                hasFiat = true
                fiatCode = entry.balance.fiatCurrencyCode
            }
        }

        let fiat: Decimal? = hasFiat ? fiatSum : nil
