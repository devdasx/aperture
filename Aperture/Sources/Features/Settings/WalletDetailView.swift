...
struct WalletDetailView: View {
    ...
    private func secretFooter(_ wallet: WalletRecord) -> LocalizedStringKey {
        ...
        case .created, .importedMnemonic:
            ...
            case .available:
                // M-008: honest crypto model — AES-GCM under a key also in local SQLite.
                return "Your recovery phrase is stored as AES-GCM ciphertext in this app’s local database. The encryption key is also kept on this iPhone (not in iCloud Keychain). Anyone with an unlocked phone and access to the app can export it after passcode/Face ID. The phrase never leaves this device on its own."
        ...
        case .importedKey:
            ...
            case .available:
                return "Your private key is stored as AES-GCM ciphertext in this app’s local database. The encryption key is also kept on this iPhone. Export still requires passcode/Face ID. The key never leaves this device on its own."
            case .encryptedRecordUnavailable:
