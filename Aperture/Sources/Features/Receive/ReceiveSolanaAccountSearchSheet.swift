...
private enum SolanaReceiveAccountDeriver {
    static func deriveMnemonicAccounts(
        ...
        for index in range {
            ...
            if let trustAddress = try? Ed25519Derivation.solanaTrustWalletAddress(seed: seed, account: account) {
                candidates.append(
                    SolanaReceiveAccountCandidate(
                        type: .trustWallet,
                        accountIndex: index,
            ...
            if let phantomAddress = try? Ed25519Derivation.solanaPhantomAddress(seed: seed, account: account) {
                candidates.append(
                    SolanaReceiveAccountCandidate(
                        type: .phantom,
                        accountIndex: index,
