...
enum KeyImportFormatDetector {
    ...
    static func detectFormat(_ raw: String, on chain: SupportedChain) -> KeyFormat? {
        ...
        case .ripple:
            // P2-015: XRPL `s…` family seeds use Ripple base58 alphabet and
            // WalletCore cannot decode them — do not classify as importable
            // `.xrpSeed` (would greenlight Continue then fail on review).
            // Only accept hex secp256k1 secrets that decodePrivateKeyBytes can.
            let body = trimmed.hasPrefix("0x") ? String(trimmed.dropFirst(2)) : trimmed
            if body.count == 64, body.allSatisfy(\.isHexDigit) {
                return .evmHex
            }
            if trimmed.hasPrefix("s"), (28...29).contains(trimmed.count),
               trimmed.allSatisfy(isBase58) {
                // Recognized shape but unsupported — unknown until decoder lands.
                return .unknown
            }
