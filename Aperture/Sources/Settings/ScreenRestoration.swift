...
/// - `ApertureApp.init()` calls `resolveOnLaunch()` before the first view is
...
/// (see `AppRoot.rootDirectionKey` in `ApertureApp.swift`); the freshly created
...
enum ScreenRestoration {
    ...
    // MARK: - Cold-launch resolution (called once, from `ApertureApp.init()`)
