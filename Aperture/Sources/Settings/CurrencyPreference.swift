...
struct SupportedCurrency: Identifiable, Hashable, Sendable {
    ...
    /// Currency symbol (`$`, `€`, `¥`, …).
    let symbol: String
    /// English display name. Localized currency names can be layered via
    /// `Locale.localizedCurrencyName(forCurrencyCode:)` when product wants
    /// picker labels in the user's selected language.
...
enum CurrencyPreference {
    ...
    /// Call once from `ApertureApp.init()` before the WindowGroup renders.
