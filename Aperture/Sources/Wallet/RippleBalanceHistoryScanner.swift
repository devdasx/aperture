...
actor RippleBalanceHistoryClient {
    ...
    private func decodePayment(
        ...
        // P2-005: incomplete payloads must not paint green "confirmed".
        // Missing TransactionResult or validated → treat as pending until known.
        let txResult = stringValue(meta["TransactionResult"])
        let validated = row["validated"] as? Bool
