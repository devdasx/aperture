...
enum SigningKeyProvider {
    ...
    static func withBitcoinKeys<R>(
        ...
        switch wallet.kind {
        case .importedKey:
            // P2-014: parity with mnemonic path — every selected UTXO owner
            // must match the single imported key's address. Fail early with
            // keyAddressMismatch instead of letting wallet-core fail later.
            let singleKeyResult: [PrivateKey] = try withImportedKey(
                wallet: wallet, chain: chain, coin: coin,
                expectedAddress: nil
            ) { key in
                let derived = bitcoinAddress(
                    privateKey: key, coin: coin, chain: chain, derivationPath: nil
                )
                if !requiredAddresses.isEmpty {
                    let uncovered = requiredAddresses.filter { $0 != derived }
            ...
            return try body(singleKeyResult)
