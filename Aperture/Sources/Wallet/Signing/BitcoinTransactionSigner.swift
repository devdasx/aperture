...
enum BitcoinTransactionSigner {
    ...
    static func sign(
        ...
        // P2-002: refuse zero/overflow rather than silently sign 0-value outs.
        let primarySats = try sats(primary.amount, decimals: draft.chain.nativeDecimals)
        ...
        if recipients.count > 1 {
            for extra in recipients.dropFirst() {
                ...
                out.amount = try sats(extra.amount, decimals: draft.chain.nativeDecimals)
    ...
    /// BUG-025 / P2-002: integer-string path (still fits Int64 for BTC-family).
    /// Throws on conversion failure or non-positive outputs so we never
    /// sign a zero-value vout from a silent `?? 0` fallback (other chains throw).
    private static func sats(_ display: Decimal, decimals: Int) throws -> Int64 {
        guard display > 0 else {
            throw SigningError.malformedDraft("Bitcoin output amount must be positive")
        }
        guard let value = SigningAmount.int64(display: display, decimals: decimals), value > 0 else {
            throw SigningError.malformedDraft("invalid Bitcoin amount (overflow or non-integral base units)")
        }
        return value
    }
