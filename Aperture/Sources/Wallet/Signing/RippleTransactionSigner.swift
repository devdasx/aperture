...
enum RippleTransactionSigner {
    ...
    static func sign(
        ...
        if draft.isTokenSend {
            ...
            // P2-011: never use NSDecimalNumber.stringValue (scientific risk).
            let iouValue = ComposeDecimal.plainDecimalString(recipient.amount)
            guard !iouValue.isEmpty, iouValue != "0", !iouValue.contains("e"), !iouValue.contains("E") else {
                throw SigningError.malformedDraft("invalid XRP IOU amount")
            }
