...
enum EVMTransactionSigner {
    ...
    static func sign(
        ...
        // P2-028: refuse crafted drafts missing fee fields rather than
        // hardcoding 30 gwei / 1.5 tip / 5 gwei (review/UI must fill fees).
        if EVMChainIdentity.usesEIP1559(draft.chain) {
            input.txMode = .enveloped
            guard var maxFee = uint64(from: draft.fee.maxFeePerGasWei), maxFee > 0 else {
                throw SigningError.malformedDraft("EVM maxFeePerGas missing — reload network fee")
            }
            guard let tip = uint64(from: draft.fee.maxPriorityFeePerGasWei), tip > 0 else {
                throw SigningError.malformedDraft("EVM priority fee missing — reload network fee")
            }
            ...
            input.txMode = .legacy
            guard let gasPrice = uint64(from: draft.fee.gasPriceWei), gasPrice > 0 else {
                throw SigningError.malformedDraft("EVM gasPrice missing — reload network fee")
            }
