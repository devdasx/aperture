...
enum SolanaTransactionSigner {
    ...
    private static func buildMultiTokenTransfer(
        ...
        // P2-025: multi SPL — refuse missing ATAs (no create-on-multi path),
        // and refuse Token-2022 / memo until single-recipient path is used.
        let memo = solanaMemo(from: draft.memo)
        if !memo.isEmpty {
            throw SigningError.signingFailed(
                "Solana multi-send with a memo isn't supported yet. Send to recipients individually, or remove the memo."
            )
        }
        if tokenProgramId != SolanaChainAdapter.splTokenProgramId {
            // Associated-token create + Token-2022 transfer-checked need
            // richer instruction encoding than the multi RawMessage path.
            throw SigningError.signingFailed(
                "Solana multi-send for this token program isn't supported yet. Send to recipients individually."
            )
        }
