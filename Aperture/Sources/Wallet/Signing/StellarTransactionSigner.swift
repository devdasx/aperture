...
enum StellarTransactionSigner {
    ...
    private static func applyMemo(_ input: inout StellarSigningInput, memo: SendMemoValue) throws {
        ...
        case .stellarMemo(.text(let text)):
            // P2-024: protocol limit is 28 **bytes**, not Unicode scalars.
            input.memoText = StellarMemoText.with { $0.text = Self.truncateMemoText(text) }
        ...
        case .text(let s):
            input.memoText = StellarMemoText.with { $0.text = Self.truncateMemoText(s) }
        default:
            input.memoVoid = StellarMemoVoid()
        }
    }

    /// Truncate to ≤28 UTF-8 bytes without splitting a multi-byte scalar.
    static func truncateMemoText(_ text: String, maxBytes: Int = 28) -> String {
        ...
        return String(decoding: utf8.prefix(end), as: UTF8.self)
    }
