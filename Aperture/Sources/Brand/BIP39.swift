...
enum BIP39 {
    ...
    ///
    /// P3-006: throws on CSPRNG failure after retries (no `fatalError`).
    /// Callers must refuse to continue without a real mnemonic — never
    /// invent zero entropy.
    static func generateMnemonic(wordCount: BIP39WordCount) throws -> [String] {
        let entropy = try secureRandomBytes(count: wordCount.entropyBytes)
        return mnemonic(fromEntropy: entropy)
    }

    enum RandomError: Error, Sendable, Equatable {
        case csprngFailed(OSStatus)
    }
    ...
    /// Wraps `SecRandomCopyBytes(kSecRandomDefault, …)` with a few retries.
    /// P3-006: throws instead of `fatalError` on OS CSPRNG failure.
    private static func secureRandomBytes(count: Int) throws -> Data {
        var lastStatus: OSStatus = errSecSuccess
        for _ in 0..<3 {
            var bytes = [UInt8](repeating: 0, count: count)
            let status = bytes.withUnsafeMutableBufferPointer { buffer -> OSStatus in
        ...
        throw RandomError.csprngFailed(lastStatus)
    }
}
