...
final class AppDatabase: @unchecked Sendable {
    ...
    /// P1-011 decision surface (pure): never destroy a live store solely
    /// because the transition marker file is missing.
    enum GRDBTransitionAction: Equatable, Sendable {
        /// Marker already present — no work.
        case noop
        /// Marker missing but a primary sqlite (or WAL) exists — rewrite
        /// marker only; leave the live database intact.
        case healMarkerPreserveStore
        /// Marker missing and no primary store files — write marker (nothing
        /// to preserve; optional orphan-sidecar cleanup only).
        case writeMarkerEmptyInstall
    }

    /// Pure policy for the one-time GRDB transition helper (P1-011).
    static func grdbTransitionAction(
        markerExists: Bool,
        primaryStoreExists: Bool
    ) -> GRDBTransitionAction {
        if markerExists { return .noop }
        if primaryStoreExists { return .healMarkerPreserveStore }
        return .writeMarkerEmptyInstall
    }

    /// True when the primary sqlite file or its WAL exists (a live store).
    static func primaryStoreFilesExist(at storeURL: URL, fileManager: FileManager = .default) -> Bool {
        let candidates = [
            storeURL,
    ...
    private static func runFreshGRDBTransitionWipeIfNeeded(
        ...
        let fm = FileManager.default
        let markerExists = fm.fileExists(atPath: markerURL.path)
        let storeExists = primaryStoreFilesExist(at: storeURL, fileManager: fm)
        let action = grdbTransitionAction(markerExists: markerExists, primaryStoreExists: storeExists)
        ...
        case .healMarkerPreserveStore:
            // P1-011: accidental marker deletion / backup restore quirks must
            // NOT re-wipe a live GRDB database (wallets/secrets).
            try writeTransitionMarker(at: markerURL, fileManager: fm)
            Logger(subsystem: "com.thuglife.aperture", category: "database").notice(
                "GRDB transition marker missing; preserved live store and rewrote marker"
            )
            return

        case .writeMarkerEmptyInstall:
            // No primary store — safe to clear orphan sidecars from a prior
            // incomplete install, then write the marker. Never purge Keychain
            // here solely for a missing marker when no store existed either
            // (nothing was "replaced").
            try deleteOrphanSidecarsOnly(at: storeURL)
            // purgeKeychain only when we actually removed something that could
            // have been a pre-GRDB handoff — not for empty-dir first launch.
            _ = purgeKeychain
            try writeTransitionMarker(at: markerURL, fileManager: fm)
            return
        }
    ...
    private static func writeTransitionMarker(at markerURL: URL, fileManager fm: FileManager) throws {
    ...
    /// Remove `-shm` / `-journal` / SwiftData-style sidecars when the primary
    /// sqlite is already gone. Does **not** delete a present primary store.
    private static func deleteOrphanSidecarsOnly(at storeURL: URL) throws {
        let fm = FileManager.default
        guard !fm.fileExists(atPath: storeURL.path) else { return }
        for fileURL in sqliteFileURLs(at: storeURL, includeSidecars: true) {
            // Never touch the primary path if it reappeared; skip if still absent.
            if fileURL.standardizedFileURL == storeURL.standardizedFileURL { continue }
