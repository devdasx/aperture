...
final class TransactionRepository {
    ...
    /// P2-003 / BUG-007: floor (not half-up) so optimistic debit never
    /// exceeds what signing will encode. Matches `ComposeDecimal.toBaseUnits`.
    private func rawBaseUnits(displayAmount: Decimal, decimals: Int) -> Decimal {
        guard displayAmount > 0, decimals >= 0 else { return 0 }
        return ComposeDecimal.toBaseUnits(displayAmount, decimals: decimals)
    }

    private func integerDatabaseText(_ value: Decimal) -> String {
        guard value > 0 else { return "0" }
        // Already integral base units after floor; plain string avoids scientific notation.
        return ComposeDecimal.plainDecimalString(value)
    }
