...
enum WalletHomeProjection {
    static func summary(db: Database, walletId: UUID, currencyCode: String) throws -> WalletHomeSummaryDTO {
        ...
        // P2-013: avoid Double intermediate (large multi-asset books lose cents).
        // Prefer row-wise Decimal sum of TEXT fiat_value_cached; fall back to numeric.
        let totalFiat = try Self.sumFiatAsDecimal(db: db, walletId: walletId, currencyCode: code)
            ?? Decimal((row?["total_fiat"] as Double?) ?? 0)
    ...
    /// P2-013: sum `fiat_value_cached` as Decimal (TEXT), not Double.
    private static func sumFiatAsDecimal(
        db: Database,
        walletId: UUID,
        currencyCode: String
    ) throws -> Decimal? {
        let values = try String.fetchAll(
            db,
            sql: """
            SELECT b.fiat_value_cached
            FROM wallet_addresses a
            JOIN token_balances b ON b.address_id = a.id
            WHERE a.wallet_id = ?
              AND b.fiat_currency_code = ?
              AND b.fiat_value_cached IS NOT NULL
              AND b.fiat_value_cached != ''
            """,
            arguments: [walletId.uuidString, currencyCode.uppercased()]
        )
        guard !values.isEmpty else { return 0 }
        var total: Decimal = 0
        for raw in values {
            guard let part = Decimal(string: raw) else { continue }
            total += part
        }
        return total
    }
