...
enum WalletSecretCrypto {
    ...
    static func seal(_ plaintext: Data, associatedData: Data) throws(CryptoError) -> Data {
        ...
        do {
            ...
            // P3-016: error type only — never key/ciphertext material.
            log.error("Wallet secret seal failed: \(String(describing: type(of: error)), privacy: .public)")
            throw .sealFailed
        }
    }
    ...
    static func open(_ ciphertext: Data, associatedData: Data) throws(CryptoError) -> Data {
        ...
        do {
            ...
            log.error("Wallet secret open failed: \(String(describing: type(of: error)), privacy: .public)")
            throw .openFailed
        }
    }
    ...
    static func clearMasterKey(database: AppDatabase = .shared) {
        try? database.write { db in
            ...
            let next = try LocalSecureBlobStore.ensureRandomKey(LocalSecureBlobStore.walletSecretMasterKey, db: db)
