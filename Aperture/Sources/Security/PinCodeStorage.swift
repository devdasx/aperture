...
enum PinCodeStorage {
    ...
    private static func _setPinSync(_ pin: String) -> Bool {
        // P3-006: fail closed if CSPRNG cannot provide salt.
        guard let salt = secureRandomBytes(count: saltLength) else { return false }
    ...
    /// platform CSPRNG via `SecRandomCopyBytes`. P3-006: returns `nil`
    /// after retries instead of `fatalError` so `setPin` can fail closed.
...
extension PinCodeStorage {
    ...
    fileprivate static func debugSmokeCheck() {
        Task.detached(priority: .utility) {
            ...
            guard let salt = secureRandomBytes(count: saltLength) else {
                assertionFailure("smoke check: CSPRNG failed")
                return
            }
            let hash = pbkdf2HmacSha256(
